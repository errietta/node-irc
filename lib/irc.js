...
function Connection (port, server, useSSL) {
  ...
  this._conn = navigator.mozTCPSocket.open(server, port, { useSSL: useSSL });
...
var net = {
  createConnection: function (port, server) {
    return new Connection(port, server, false);
  },
};
var tls = {
  connect: function (port, server, creds, cb) {
...
Client.prototype.addListener = function (evt, cb) {
  ...
  this.eventEmitter.addEventListener(evt, function (data) {
    //console.log('heard a ' + evt + ' event');
    //console.log(JSON.stringify(data.detail));
    cb.apply(this, data.detail);
  });
};
Client.prototype.emit = emit;
Client.prototype.once = once;
